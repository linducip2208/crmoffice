...
<head>
<meta charset="utf-8">
<title>Estimasi {{ $estimate->number }}</title>
<style>
...
<div class="header">
  ...
  <div class="meta">
    <div><strong>Nomor:</strong> {{ $estimate->number }}</div>
    <div><strong>Tanggal:</strong> {{ $estimate->estimate_date?->format('d M Y') }}</div>
    <div><strong>Berlaku Hingga:</strong> {{ $estimate->expiry_date?->format('d M Y') ?? '—' }}</div>
...
<table class="totals">
  ...
  @if($estimate->discount_total > 0)
    <tr><td class="label">Diskon</td><td class="val">− {{ $estimate->currency->symbol ?? 'Rp' }} {{ number_format($estimate->discount_total, 2, ',', '.') }}</td></tr>
  @endif
  <tr><td class="label">Pajak</td><td class="val">{{ $estimate->currency->symbol ?? 'Rp' }} {{ number_format($estimate->tax_total, 2, ',', '.') }}</td></tr>
  <tr class="grand"><td class="label">Total</td><td class="val">{{ $estimate->currency->symbol ?? 'Rp' }} {{ number_format($estimate->total, 2, ',', '.') }}</td></tr>
</table>

@if($estimate->notes)
  <div class="notes"><strong>Catatan</strong><br>{{ $estimate->notes }}</div>
@endif

@if($estimate->terms)
  <div class="notes"><strong>Syarat & Ketentuan</strong><br>{{ $estimate->terms }}</div>
@endif

...
<div class="grid">
  <div class="col">
    ...
    @endif
    @if($invoice->client->tax_id)<div class="muted">Tax ID: {{ $invoice->client->tax_id }}</div>@endif
  </div>
  <div class="col" style="text-align:right">
    ...
    @if($invoice->status !== 'paid' && $invoice->due_date)
      <div class="muted" style="font-size:10px;margin-top:4px">Due {{ $invoice->due_date->format('d M Y') }}</div>
    @endif
  </div>
</div>
...
<table class="totals">
  ...
  @if($invoice->discount_total > 0)
    <tr><td class="label">Discount</td><td class="val">− {{ $invoice->currency->symbol ?? 'Rp' }} {{ number_format($invoice->discount_total, 2, ',', '.') }}</td></tr>
  @endif
  <tr><td class="label">Tax</td><td class="val">{{ $invoice->currency->symbol ?? 'Rp' }} {{ number_format($invoice->tax_total, 2, ',', '.') }}</td></tr>
  ...
  @if($invoice->paid_total > 0)
    <tr><td class="label">Paid</td><td class="val">− {{ $invoice->currency->symbol ?? 'Rp' }} {{ number_format($invoice->paid_total, 2, ',', '.') }}</td></tr>
    <tr><td class="label" style="font-weight:700">Balance Due</td><td class="val" style="font-weight:700">{{ $invoice->currency->symbol ?? 'Rp' }} {{ number_format($invoice->balance_due, 2, ',', '.') }}</td></tr>
  @endif
</table>
...
@if($invoice->terms)
  <div class="notes"><strong>Terms</strong><br>{{ $invoice->terms }}</div>
@endif
